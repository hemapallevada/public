...
if(ans1==false){
    document.write("<html><body id='bd' ><center><h2>Please register as Customer</h2></center></body></html>");
...
async function uploadhash(){
        ...
        var1="https://ipfs.infura.io/ipfs/"+var1;
        const ans = await window.contract.methods.signup_as_vehicle_owner(var1,mob,account).send({ from: account });
        document.write("<html><body id='bd' ><center><h2>Uploading License is done</h2></center></body></html>");
style='background-image: url("bg2.jpg");'
document.getElementById('bd').style.backgroundImage="url(bg2.jpg)";
...
async function signupcust(){
    ...
    const ans = await window.contract.methods.signup_as_user(mob,account).send({ from: account });
    document.write("<html><body id='bd' ><center><h2>Customer Registration Done</h2></center></body></html>");
style='background-image: url("bg2.jpg");'
document.getElementById('bd').style.backgroundImage="url(bg2.jpg)";
...
document.write("<html><body id='bd' ><center><h2>Please register as Vehicle</h2></center></body></html>");
style='background-image: url("bg2.jpg");'
document.getElementById('bd').style.backgroundImage="url(bg2.jpg)";
return;
}
...
var arr_of_names=["first","second","third","fourth","fifth","sixth","seventh","eigth","nineth","tenth"];
for(var i=0;i<10;i++){
...
for(var i=0;i<10;i++){
if(allUndropped[i]!="0"){
  var firstobj=document.getElementById(arr_of_names[i]+"_"+"bookedId");
    firstobj.innerHTML=allUndropped[i];
var sourceobj=document.getElementById(arr_of_names[i]+"_"+"source");
sourceobj.innerHTML="<label>Source&nbsp&nbsp</label>"+ await getSource(parseInt(allUndropped[i]));
var desobj=document.getElementById(arr_of_names[i]+"_"+"dest");
desobj.innerHTML="<label>Dest &nbsp&nbsp&nbsp&nbsp&nbsp</label>"+ await getDestination(parseInt(allUndropped[i]));
var mobobj=document.getElementById(arr_of_names[i]+"_"+"mob");
mobobj.innerHTML="<label>Mobile&nbsp&nbsp&nbsp</label>"+ await getMobile(parseInt(allUndropped[i]));
...
const ans = await window.contract.methods.drop_accepted(a).send({ from: account });
}
...
async function addPickUp(a,b,c){
    ...
    var weival=c*100;
    const account = await getCurrentAccount();
    const drop_id = await window.contract.methods.picked_up(a,b,c,account).send({ from: account });
    if(drop_id==false){
